...
export default function InviteBar({ gameId }: params) {
                    ...
                    return (
                        <Popover.Root key={`offline-friend-${index}`}>
                            <Popover.Trigger asChild>
                                <div className="flex items-center p-2 border border-transparent hover:border-[var(--foreground)] cursor-pointer rounded-xl">
                            ...
                            <Popover.Portal>
                                <Popover.Content
                                    ...
                                    sideOffset={8}
                                >
                                    <p className="font-bold text-[var(--foreground)]">{friend.username}</p>
                                    <p className="text-sm text-[var(--color2)]">Status: Online</p>
                                    <button
                                        onClick={() => inviteUser(friend)}
                                        className="mt-2 bg-[var(--foreground)] hover:bg-[var(--color2)] text-[var(--background)] px-3 py-1 rounded text-sm w-full border-none outline-none focus:ring-0"
                                    >
                                        Invite to Game
                                    </button>
